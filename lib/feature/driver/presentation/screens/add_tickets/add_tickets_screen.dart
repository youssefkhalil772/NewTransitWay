...
class _AddTicketsScreenState extends State<AddTicketsScreen>
    ...
    if (_busId != null && _busId!.isNotEmpty) {
      try {
        ...
        if (activeTripRes.isNotEmpty) {
          activeTripStart = DateTime.tryParse(
            activeTripRes.first['created_at'].toString(),
          );
        }
      } catch (_) {}
    }
    ...
    _ticketSubscription = SupabaseConfig.client
        ...
        .listen(
          (data) async {
            ...
            if (validTickets.length > _allTickets.length &&
  ...
  Future<List<Map<String, dynamic>>> _enrichTicketsLocally(
  ...
  List<Map<String, dynamic>> get _activeTickets {
    ...
    return _allTickets.where((t) {
      ...
      return tTime.isAfter(_activeTripStart!) ||
          tTime.isAtSameMomentAs(_activeTripStart!);
    }).toList();
  }
  ...
  Widget _buildBody() {
    final activeCount = _activeTickets
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'All tickets issued for your bus',
                ...
                child: _isLoadingTickets
                    ...
                    : TabBarView(
                        ...
                        children: [
                          _buildTicketList(
                            _activeTickets,
                            'No tickets for active trip',
                            'Tap "Issue Tickets" to add manual tickets',
                            key: const ValueKey('active_tab'),
                          ),
                          _buildTicketList(
                            _historyTickets,
                            'No tickets yet',
                            'You have not issued any tickets.',
    ...
    return RefreshIndicator(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (_, i) {
          ...
          final price =
              ((t['routes'] as Map?)?['price'] as num?)?.toStringAsFixed(0) ??
              '0';
          final busNumber =
              (t['buses'] as Map?)?['bus_number']?.toString() ?? '---';
          final rawStatus = t['status']?.toString() ?? 'active';
          final status = rawStatus.toLowerCase() == 'active'
              ? 'Sold'
              : rawStatus;
...
class _IssueTicketsSheetState extends State<_IssueTicketsSheet> {
  ...
  static const Map<String, String> _arabicErrors = {
    'No active trip for this bus':
        'Ù…ÙÙŠØ´ Ø±Ø­Ù„Ø© Ù†Ø´Ø·Ø© Ø¯Ù„ÙˆÙ‚ØªÙŠØŒ Ø§Ø¨Ø¯Ø£ Ø±Ø­Ù„Ø© Ø£ÙˆÙ„Ø§Ù‹',
    'No bus assigned to driver': 'Ù…ÙÙŠØ´ Ø¨Ø§Øµ Ù…ØªØ¹ÙŠÙ† Ù„ÙŠÙƒ',
    'Invalid ticket price': 'Ø§Ù„Ø³Ø¹Ø± Ø£Ùˆ Ø§Ù„Ø®Ø· ØºÙŠØ± ØµØ§Ù„Ø­',
    'driverId and numberOfTickets are required':
        'Ø¨ÙŠØ§Ù†Ø§Øª Ù†Ø§Ù‚ØµØ©ØŒ Ø­Ø§ÙˆÙ„ ØªØ§Ù†ÙŠ',
  };

  String _translateError(String? error) {
    if (error == null) return 'Ø­ØµÙ„ Ø®Ø·Ø£ ØºÙŠØ± Ù…ØªÙˆÙ‚Ø¹';
    ...
    try {
      ...
      if (currentBusId == null || currentBusId.isEmpty) {
        final driverData = await SupabaseConfig.client
            ...
            .maybeSingle();
        currentBusId =
            driverData?['busId']?.toString() ??
            driverData?['current_bus_id']?.toString();
        if (currentBusId == null || currentBusId.isEmpty) {
          throw 'Ù…ÙÙŠØ´ Ø¨Ø§Øµ Ù…ØªØ¹ÙŠÙ† Ù„ÙŠÙƒ';
        }
      }
      ...
      if (tripData == null || tripData['route_id'] == null) {
        throw 'Ù…ÙÙŠØ´ Ø±Ø­Ù„Ø© Ù†Ø´Ø·Ø© Ø¯Ù„ÙˆÙ‚ØªÙŠØŒ Ø§Ø¨Ø¯Ø£ Ø±Ø­Ù„Ø© Ø£ÙˆÙ„Ø§Ù‹';
      }
      ...
      for (int i = 0; i < count; i++) {
        ...
        ticketsToInsert.add({
          'user_id':
              null, // manual tickets don't belong to a passenger app user
          'bus_id': currentBusId,
          'route_id': routeId,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 6.h),
          Text(
            'Enter the number of tickets, the price will be determined from the active route.',
            style: TextStyle(fontSize: 12.sp, color: const Color(0xff6B7C6E)),
          ),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(
                _isLoading ? 'Issuing...' : 'Issue Tickets',
                style: TextStyle(
                  fontSize: 15.sp,
                  fontWeight: FontWeight.bold,
                  color: Colors.white,
      ...
      children: [
        ...
        Text(
          ...
          style: TextStyle(
            ...
            color: bold ? _green : Colors.black87,
          ),
        ),
      ],
    );
