...
class _ReportComplaintScreenState extends State<ReportComplaintScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (_descController.text.trim().length < 10) {
      setState(
        () =>
            _errorMsg = 'Please describe the problem (at least 10 characters).',
      );
      return;
    }
    ...
    try {
      ...
      final fileName =
          'original_${ts}_${_imageFile!.path.split('\\').last.split('/').last}';
      final path = fileName;
      ...
      debugPrint("ðŸ“¸ Complaint Image Uploaded: $imageUrl");
      ...
      try {
        final aiResponse = await HttpClient()
            .postUrl(Uri.parse('http://54.91.157.86:8000/predict'))
            .then((request) {
              request.headers.contentType = ContentType.json;
              request.write(
                '{"image_url": "$imageUrl", "description": "${_descController.text.trim()}"}',
              );
        ...
        if (aiResponse.statusCode == 200) {
          ...
          debugPrint("ðŸ¤– AI Analysis Complete: $data");
        } else {
          debugPrint("ðŸ¤– AI Error: Status ${aiResponse.statusCode}");
        }
      } catch (e) {
        debugPrint("ðŸ¤– AI Request Failed: $e");
      }
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                'Your report goes directly to our admin team.',
