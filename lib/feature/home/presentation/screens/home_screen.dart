...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  Future<void> _initializeData() async {
    ...
    UserDataManager().startRealtime();

    _allStations = await UserDataManager().getStations();

    _loadInitialLocation();
    _setupRealtimeBuses();
    _setupRealtimeStations();
  ...
  void _setupRealtimeStations() {
    _stationsSubscription?.cancel();
    _stationsSubscription = UserDataManager().stationsStream.listen((
      updatedStations,
    ) {
      if (!mounted) return;
      setState(() {
        ...
        .listen((data) {
          ...
          for (final b in data) {
            ...
            if (lat != null && lng != null) {
              _activeBuses[b['id'].toString()] = LatLng(
                (lat as num).toDouble(),
                (lng as num).toDouble(),
              );
            }
  ...
  void _triggerRouteDrawing() async {
    ...
    List<StationModel> zoneStations = _allStations
        .where((s) => s.zone == _selectedFromStation!.zone)
        .toList();
    int startIndex = zoneStations.indexWhere(
      (s) => s.id == _selectedFromStation!.id,
    );
    int endIndex = zoneStations.indexWhere(
      (s) => s.id == _selectedToStation!.id,
    );
    if (startIndex == -1 || endIndex == -1) return;

    int actualStart = startIndex < endIndex ? startIndex : endIndex;
    int actualEnd = startIndex < endIndex ? endIndex : startIndex;
    List<StationModel> segment = zoneStations.sublist(
      actualStart,
      actualEnd + 1,
    );
    ...
    if (routeWaypoints.length >= 2) {
      ...
      _mapController.fitCamera(
        CameraFit.bounds(bounds: bounds, padding: EdgeInsets.all(70.w)),
      );
    }
  ...
  Future<void> _handleFindTrip() async {
    ...
    try {
      final response = await _repository.getNearestBus(
        _selectedFromStation!.id,
      );

      if (response == null || (response is List && response.isEmpty)) {
        throw Exception(
          "No buses available on ${_selectedFromStation!.zone} route right now. Please try again later.",
        );
      }
      ...
      final String busNum =
          data['bus_number']?.toString() ??
          data['busNumber']?.toString() ??
          'Unknown';
      final int eta = data['eta_minutes'] is int
          ? data['eta_minutes']
          : int.tryParse(data['eta_minutes']?.toString() ?? '') ?? 5;
      ...
      if (mounted) {
        final result = await Navigator.pushNamed(
          ...
          arguments: {
            ...
            'arrivalTime': "$eta min",
            'distance':
                data['distanceToStationKm'] ??
                data['distance_to_station_km'] ??
                data['distance_km'],
            'stations': allRouteStations
                .map(
                  (s) => {'name': s.name, 'latLong': s.latLong, 'zone': s.zone},
                )
                .toList(),
        ...
        if (result == "OPEN_QR" && widget.onScanRequested != null)
  ...
  Widget _buildContent() {
    return Stack(
      children: [
        _buildMap(),
        Positioned(top: 100.h, right: 20.w, child: _buildLocationButton()),
            ...
            PolylineLayer(
              polylines: [
                Polyline(
                  ...
                  color: RouteModel.getColorFromName(
                    _selectedFromStation?.zone ?? "",
                  ),
                  strokeWidth: 4.0,
                ),
              ],
  ...
  Marker _buildUserLocationMarker() => Marker(
    point: _userLocation!,
    width: 40,
    height: 40,
    child: Stack(
  ...
  Marker _buildStationMarker(StationModel station) => Marker(
    ...
    child: GestureDetector(
      ...
      child: Container(
        ...
        child: Padding(
          ...
          child: Container(
            decoration: BoxDecoration(
              color: RouteModel.getColorFromName(station.zone),
              shape: BoxShape.circle,
            ),
            child: const Icon(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.location_on, color: color, size: 45.sp),
  ...
  Widget _buildSearchField(
    ...
    readOnly: true,
    onTap: () => _showStationPicker(controller, isFrom),
          ...
          builder: (context, setModalState) {
            ...
            if (!isFrom && _selectedFromStation != null) {
              stationsToDisplay = _allStations
                  .where(
                    (s) =>
                        s.zone == _selectedFromStation!.zone &&
                        s.orderIndex > _selectedFromStation!.orderIndex,
                  )
                  .toList();
            }
            List<StationModel> filtered = stationsToDisplay
                .where(
            ...
            return Container(
              ...
              child: Column(
                children: [
                  ...
                  if (!isFrom && _selectedFromStation != null)
                    Text(
                      "Route: ${_selectedFromStation!.zone}",
                      style: TextStyle(
                        color: RouteModel.getColorFromName(
                          _selectedFromStation!.zone,
                        ),
                    ...
                    child: filtered.isEmpty
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final rColor = RouteModel.getColorFromName(
                                station.zone,
                              );
                              return ListTile(
